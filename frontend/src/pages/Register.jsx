...
const Register = () => {
  ...
  const inputCls = "w-full pl-11 pr-4 py-3.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 focus:border-purple-400 dark:focus:border-purple-500 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-purple-500/20 transition-all";

  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex transition-colors duration-300">

      {/* ── Left decorative panel ── */}
      <div className="hidden lg:flex w-1/2 relative overflow-hidden bg-gradient-to-br from-blue-50 dark:from-blue-950 to-slate-100 dark:to-slate-950 items-center justify-center p-12">
        <div className="absolute inset-0">
          <div className="absolute top-1/3 left-1/4 w-64 h-64 bg-blue-300/30 dark:bg-blue-600/20 rounded-full blur-3xl animate-pulse" />
          <div className="absolute bottom-1/4 right-1/4 w-48 h-48 bg-purple-300/30 dark:bg-purple-600/20 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1.5s' }} />
          ...
          <div className="space-y-3">
            {['Free to join, no credit card needed', 'Instant token wallet on registration', 'Secure JWT authentication', 'Role-based access control'].map((item, i) => (
              <div key={i} className="flex items-center gap-3 text-slate-600 dark:text-slate-300 text-sm">
                <div className="w-5 h-5 rounded-full bg-green-100 dark:bg-green-500/20 border border-green-300 dark:border-green-500/40 flex items-center justify-center flex-shrink-0">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 overflow-y-auto">
        <div className="w-full max-w-md py-8">
          ...
          <p className="text-slate-500 dark:text-slate-400 mb-8">Join TokenTrackr and start earning rewards</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {success && (
              <div className="flex items-center gap-3 bg-green-50 dark:bg-green-500/10 border border-green-200 dark:border-green-500/30 text-green-700 dark:text-green-400 rounded-xl px-4 py-3 text-sm">
            ...
            <div className="group">
              ...
              <div className="relative">
                ...
                <input id="register-password" type={showPw ? 'text' : 'password'} name="password" value={formData.password} onChange={handleChange} required minLength={6} placeholder="Min. 6 characters"
              ...
              <div className="grid grid-cols-2 gap-3">
                {roles.map((r) => (
                  ...
                  >
                    ...
                    <div className="text-slate-900 dark:text-white font-bold text-sm">{r.label}</div>
                    <div className="text-slate-500 dark:text-slate-400 text-xs mt-1">{r.desc}</div>
                    {formData.role === r.value && (
