...
const InputField = ({ id, label, type, name, value, onChange, placeholder, icon }) => (
  <div className="group">
...
const Login = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex transition-colors duration-300">
      {/* ── Left Panel ── */}
      <div className="hidden lg:flex w-1/2 relative overflow-hidden bg-gradient-to-br from-purple-100 dark:from-purple-950 to-slate-100 dark:to-slate-950 items-center justify-center p-12">
        <div className="absolute inset-0">
          <div className="absolute top-1/4 left-1/4 w-64 h-64 bg-purple-600/20 rounded-full blur-3xl animate-pulse" />
          <div className="absolute bottom-1/4 right-1/4 w-48 h-48 bg-blue-600/20 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />
        ...
        <div className="relative z-10 max-w-sm text-center">
          <div className="w-20 h-20 bg-gradient-to-br from-purple-500 to-blue-600 rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-2xl shadow-purple-500/40 animate-float">
            <svg className="w-10 h-10 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z" />
            </svg>
          </div>
          <h2 className="text-3xl font-black text-slate-900 dark:text-white mb-4">Token<span className="text-purple-500">Trackr</span></h2>
          <p className="text-slate-600 dark:text-slate-400 text-lg leading-relaxed mb-8">Show up. Earn tokens. Redeem rewards. Your attendance has never been more valuable.</p>
          <div className="grid grid-cols-3 gap-3">
            {[
              { icon: '⚡', label: '10 tokens/day' },
              { icon: '🏆', label: 'Earn rewards' },
              { icon: '📊', label: 'Track progress' },
            ].map((item, i) => (
              <div key={i} className="bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl p-3 text-center hover:bg-slate-200 dark:hover:bg-white/10 transition-colors">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6">
        <div className="w-full max-w-md">
          <div className="mb-8">
            ...
            <p className="text-slate-500 dark:text-slate-400">Sign in to continue your token journey</p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-5">
            {error && (
              <div className="flex items-center gap-3 bg-red-500/10 border border-red-500/30 text-red-400 rounded-xl px-4 py-3 text-sm animate-shake">
          ...
          <p className="text-center text-slate-500 text-sm mt-8">
            ...
            <Link to="/register" className="text-purple-400 hover:text-purple-300 font-semibold transition-colors">Create one free →</Link>
          </p>
        </div>
      </div>
    </div>
